...
\newcommand{\vida}[1]{{\color{pink} Vida: #1}}
...
\david{please check your affiliations and support statements} \pat{Mine is good.} \david{Gwen: Are you still supported by the Australian Research Council?}
\vida{I guess I am now supported by them, ARC} \david{I think everyone should write their own support statements.}
...
This paper studies the global structure of planar graphs and more general graph classes, through the lens of graph blowups. Here, the \defin{$k$-blowup} of a graph $H$ is the graph obtained by replacing each vertex $v$ of $H$ with a complete graph $K_v$ of order $k$ and replacing each edge $vw$ of $H$ with a complete bipartite graph with parts $V(K_v)$ and $V(K_w)$, as illustrated in \cref{2blowup}. We consider the following question: What is the simplest family of graphs $\mathcal{H}$ such that, for each $n$-vertex planar graph $G$, there is a graph $H\in\mathcal{H}$ such that $G$ is contained in a $\tilde{O}(\sqrt{n})$-blowup of $H$, where $\tilde{O}$ notation hides $\polylog(n)$ terms?\footnote{We say that a graph $G$ is \defin{contained} in a graph $G'$ if $G$ is isomorphic to a subgraph of $G'$.} We show that one can  take $\mathcal{H}$ to be the class of fans, where a \defin{fan} is a graph consisting of a path $P$ plus one vertex that dominates $P$.
...
%\david{Currently we discuss $k$-blowups of $H$, $H$-partititons of width $k$, and subgraphs of $H\boxtimes K_k$. They are equivalent. Do we need to mention all three? We need to define $\boxtimes$ because of $H\boxtimes P$. Do we need to mention partitions? We could scrap \cref{bandwidth_relation}, and in \cref{Bandwidth} we could replace the connection between bandwidth and path-partitions, by the equivalent connection between bandwidth and blowups of paths. Partitions aren't mentioned elewhere. What do you think? We could even move the revised \cref{Bandwidth} to the paragraph before \cref{main_thm_planar_restated}.  What do you think?}

%\pat{I agree, let's drop the discussion of $\mathcal{H}$-partitions} \david{i'm on it.}

%\pat{I agree again, let's move the discussion of bandwidth in \cref{bandwidth_relation} so that it's between \cref{main_thm_planar} and \cref{main_thm_planar_restated}. }

\begin{figure}[h]
   \centering
   \includegraphics{figs/fan-blowup}
    \caption{A 5-blowup of a fan.}
    \label{2blowup}
\end{figure}

\david{I suggest the circle boundary of a vertex in the fan be a different colour from the colour of the fan edges. Then use the same colour for the boundary of the blowup. }

%\david{This figure refers to the strong product, but the connection between blowups and strong products is not mentioned until much later.} \pat{Yes, I wanted to draw the original fan in the same style as the blowup.  I was hoping the reader could deduce that the thing on the left somehow represents a $5$-blowup operation, and the $\boxtimes$ symbol would make sense later.} \david{okay}

% \david{perhaps this paragraph should go in the opening subsection, what do you think?}
The blowup factor in this result is close to best possible. A $k$-blowup of a graph $H$ with treewidth\footnote{A \defin{tree-decomposition} of a graph $G$ is a collection $(B_x)_{x \in V(T)}$ of subsets of $V(G)$ indexed by a tree $T$, such that: (a) for every edge ${vw \in E(G)}$, there exists a node ${x \in V(T)}$ with ${v,w \in B_x}$, and (b) for every vertex ${v \in V(G)}$, the set $\{ x \in V(T) \colon v \in B_x \}$ induces a non-empty (connected) subtree of $T$. The \defin{width} of such a tree-decomposition is ${\max\{ |B_x| \colon x \in V(T) \}-1}$. A \defin{path-decomposition} is a tree-decomposition where the underlying tree is a path, denoted by the corresponding sequence of bags. The \defin{treewidth $\tw(G)$} of a graph $G$ is the minimum width of a tree-decomposition of $G$. The \defin{pathwidth $\pw(G)$} of a graph $G$ is the minimum width of a path-decomposition of $G$. Treewidth is the standard measure of how similar a graph is to a tree. Pathwidth is the standard measure of how similar a graph is to a path. By definition, $\tw(G)\leq\pw(G)$ for every graph $G$.} $t$ has treewidth at most $k(t+1)-1$.  Since there are $n$-vertex planar graphs of treewidth $\Omega(\sqrt{n})$ (such as the $\sqrt{n}\times\sqrt{n}$ grid), any result like \cref{main_thm_planar} that finds all planar graphs in blowups of bounded treewidth graphs must have blowups of size $\Omega(\sqrt{n})$
(and fans have treewidth 2, in fact pathwidth 2). 
...
The \defin{bandwidth} of $G$ is $\mathdefin{\bw(G)}:=\min\{\bw_G(v_1,\ldots,v_n):\text{$v_1,\ldots,v_n$ is an ordering of $V(G)$}\}$. See \citep{CS89,ST20,ABET20,BPTW10,rao:small,BST09,feige:approximating} for a handful of important references on this topic. 
%
It is well known~\citep{DSW07,BPTW10} that bandwidth is closely related to blowups of paths.  Indeed, if $\bw_G(v_1,\ldots,v_n)=w$, then mapping 
$v_{(i-1)w+1},\dots, v_{iw}$ to the $i$-th vertex of a path, we see that $G$ is contained in the $w$-blowup of a path on $\lceil n/w\rceil$ vertices. In the other direction, if $G$ is contained in a $w$-blowup of a path $P$, and $B_i$ is the set of vertices mapped to the $i$-th vertex of $P$, then any ordering $v_1,\ldots,v_n$ of $V(G)$ that places all vertices of $B_i$ before those in $B_{i+1}$ for each $i$ has $\bw_G(v_1,\ldots,v_n)\le 2w-1$. Thus, \cref{main_thm_planar} is equivalent to the following, where $X$ is the set of vertices mapped to the dominant vertex of the fan:
...
\begin{table}[!ht]
\caption{Results on $k$-blowups of a graph $H$ that contain every $n$-vertex graph $G$ from graph class $\mathcal{G}$.}
\label{results_table}
\ \\[.5ex]
\centering
...
\midrule
    ...
    & \textcolor{red}{$\Omega(t\sqrt{n})$} & \cite{ast90}
...
\midrule
    ...
    apex-$X$-minor-free & fan
    & $\Omega(\sqrt{n})$ &
    & $O_X(\sqrt{n}\log^2 n)$ & \cref{main_thm_apexmf}\\
\midrule
...
\david{I suggest the entries for $K_{3,t}$-minor-free graphs and apex-minor-free graphs be grouped together.} \pat{I implemented that, but now I see that it might be confusing for readers, since the $A$-minor-free (for apex $A$) upper bound is really $O_A(\sqrt{n}\log^2 n)$.} \david{how about now?}

\david{ $k$-planar and $k$-blowup are incompatible in this table, shall we write $b$-blowup?}
...
\david{note that the table enrty for $(g,k)$-planar graphs is a small lie, but it is explained in the theorem statement, so I can live with it. Okay?}
...
\david{I now think these entries should stay in the same table, but I would like to keep the following paragraph. Okay?} \pat{Okay with me.} \david{Gwen, okay with you?}
...
We now explain why, except possibly for the $\log^2 n$ factor, \cref{main_thm_planar} cannot be strengthened.  As already discussed in the introduction, a factor of $\sqrt{n}$ in the size of the blowup is necessary, since there are $n$-vertex planar graphs of treewidth $\Omega(\sqrt{n})$.
...
We in fact prove the following generalization of \cref{main_thm_planar,main_thm_k_planar}. Here a graph $G$ is \defin{$(g,k)$-planar} if it has a drawing in a surface of Euler genus at most $g$ in which each edge is in at most $k$ crossings, and no three edges cross at the same point. 
...
\begin{lem}
\label{genus_D}
For any $D\in\R$ and $n\in\NN$ with $1\leq D\leq n$, any $n$-vertex graph $G$ of Euler genus $g$ has a set $X$ of $O(\sqrt{gn}+(n\log n)/D)$ vertices such that $G-X$ has bandwidth at most $O(D\log^3 n)$.
\end{lem}

\begin{proof}
By \cref{eppstein_planarizer}, $G$ has a set $X_0$ of $O(\sqrt{gn})$ vertices such that $G-X_0$ is planar.  By \cref{main_thm_planar_gen}, $G-X_0$ has a set $X_1$ of $O((n\log n)/D)$ vertices such that $G-(X_0\cup X_1)$ has bandwidth at most $O(D\log^3 n)$. The result follows taking $X:=X_0\cup X_1$.
\end{proof}
...
For any $D\in\R$ and $n\in\NN$ with $1\leq D\leq n$, any $n$-vertex $k$-planar graph $G$ has a set $X$ of $O((k^{3/2}n\log n)/D)$ vertices such that $G-X$ has bandwidth at most $O(kD\log^3 n)$.
\end{lem}
...
\begin{lem}
\label{gkPlanar}
For any integers $g,k\geq 0$ and $n\geq 1$, for any $D\in\R$ with $1\leq D\leq n$, any $n$-vertex $(g,k)$-planar graph $G$ has a set $X$ of $O((k+1)^{3/4}(g+1)^{3/4}n^{1/2} + ((k+1)^{3/2}(g+1)^{1/2}n\log n)/D)$ vertices such that $G-X$ has bandwidth at most $O(D(k+1)\log^3 n)$.
\end{lem}
...
& \subseteq O((k+1)^{3/4}(g+1)^{3/4}n^{1/2} + ((k+1)^{3/2}(g+1)^{1/2}n \log((k+1)^{3/2}(g+1)^{1/2}n))/D)\\
& \subseteq O((k+1)^{3/4}(g+1)^{3/4}n^{1/2} + ((k+1)^{3/2}(g+1)^{1/2}n\log n)/D) .
\end{align*}
...
\begin{enumerate}
  ...
  \item Prove that $\mathcal{M}^*$ has a $(k,O(\sqrt{\log n}))$-volume-preserving Euclidean contraction $\phi:V(G-X)\to\R^L$, for $k\in\Theta(\log n)$.  The preceding two steps are done in such a way that this part of the proof is able to closely follow the proof of \cref{rao_planar_graphs} by \citet{rao:small}.
...
\subsubsection{\boldmath The Distance Function $d^*$}
...
\subsubsection{Volume-Preserving Contraction of $\mathcal{M}^*$}
...
\begin{proof}
  ...
  Let $S':=\{j\in S:  |\phi_{i,j}(v_p)-x_{i,j}|\ge \Delta/16\}$.  Then $|S'|$ dominates a $\operatorname{binomial}(|J|,1/2)$ random variable.  By the Chernoff Bound (and the union bound), for all $a\ge 192$, $n\ge 2$, and $k\ge 2$,\pat{TODO: Recompute these dumb lower bounds. Constants have changed.}
  \[
...
\begin{proof}[Proof of \cref{dstar_contraction}]
  ...
  \[
    \eta = \frac{3\sqrt{L}}{\zeta} = \frac{3\cdot 640\sqrt{2L}}{\sqrt{ak\ln n}} = {1920\sqrt{2\floor{1+\log n}}} = O(\sqrt{\log n}) \enspace . \qedhere
  \]
\end{proof}
...
\cref{main_thm_products} follows from \cref{main_thm_products_D} by taking $D:=\sqrt{tn}/\log n$.
