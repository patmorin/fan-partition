...
\todo[inline]{Somewhere explain that $\sqrt{n}$ is the magic number here because planar graphs have treewidth up to, but not exceeding $\Theta(\sqrt{n})$.  Explain why we can have tree-partitions of width $O(\sqrt{n})$ (the graph $F_n$ is a good reason).}
