...
This definition is consistent with the definition of local density of graphs:  A connected graph $G$ has local density at most $D$ if and only if the metric space $\mathcal{M}_G$ has local density at most $D$.  Note that, if $(S,d)$ has local density at most $D$ then $(S,d)$ \david{has} $\diam_d(S)\ge (|S|-1)/D$ and $\mindist(S)\ge 1/D$.
...
First, we need a definition of bandwidth for metric spaces.  Let $(S,d)$ be a \david{finite?} metric space and let $x_1,\ldots,x_n$ be a permutation of $S$.  Then $\mathdefin{\bw_{(S,d)}(x_1,\ldots,x_n)}:=\max\{j-i:d(x_i,x_j)\le 1\}$ \david{should this be $|j-i|$?} and $\mathdefin{\bw(S,d)}$ is the minimum of $\bw_{(S,d)}(x_1,\ldots,x_n)$ taken over all $n!$ permutations $x_1,\ldots,x_n$ of $S$.  Note that this coincides with the definition of the bandwidth of a graph: For any connected graph $G$, $\bw(V(G),d_G)=\bw(G)$.  First we observe that injective \david{given that `injective' does not appear in the observation, why do we have `injective' here?} contractions can only increase bandwidth:
...
\begin{lem}\label{planar_sparsifier}
  For each $1\le D\le n$ \david{many readers will think this means that $D$ is in $\{1,2,\dots,n\}$, 
  so I suggest we start ``For any $D\in\mathbb{R}$ with $1\leq D\leq N$''}, every $n$-vertex planar graph $G$ has a set $X$ of $O((n/D)\log n)$ vertices such that $G-X$ has local density at most $D$.
\end{lem}
...
\begin{lem}\label{weighted_separator}
    Let $H$ be a graph; let $\mathcal{T}:=(B_x:x\in V(T))$ be a tree decomposition of $H$; and let $\xi:V(H)\to\R$ be a function that is positive on $V(H)$. \david{I suggest we define
      $\xi(X):=\sum_{v\in V(X)} \xi(v)$ for any subgraph $X$ of $H$, and use throughout, including replacing $\Xi$ by $\xi(H)$.} Then, for any $c\in\N\setminus\{0\}$, there exists $S_T\subseteq V(T)$ of size $|S_T|\le c-1$ such that, for each component $X$ of $H-(\bigcup_{x\in S_T} B_x)$, $\sum_{v\in V(X)} \xi(v) \le \tfrac{1}{c}\cdot\sum_{v\in V(H)} \xi(v)$. \david{replace $S_T$ by $S$?}
\end{lem}
...
\david{At the start of this section we promised a generalization of \cref{planar_sparsifier}. So at this point, should we prove the following: Let $G$ be an $n$-vertex graph that has a layering such that the subgraph induced by the union of any $r$ consecutive layers has treewidth at most $cr-1$. Then for any $D\in\mathbb{R}$ with $1\leq D\leq N$, $G$ has a set $X$ of $O((n/D)\log n)$ vertices such that $G-X$ has local density at most $D$. We will need this later.}


\begin{proof}[Proof of \cref{planar_sparsifier}]
  If $G$ is not connected, add edges to create a connected planar supergraph $G^+$ of $G$. If there exists $X$ of size $O(n/D)$ such that $G^+-X$ has local density at most $D$ then, by \cref{contraction_increases_density,supergraph_contraction}, $G-X$ has local density at most $D$. 
 Thus, it suffices to consider the case where $G$ is connected.

 Let $v_0$ be any vertex of $G$ and, for each integer $s$, let $L_s:=\{v\in V(G): d_G(v_0,v)=s\}$, and let $h:=\max\{s:|L_s|>0\}$.  ($L_0,\ldots,L_h$ is known as a BFS layering of $G$.) It is a well known property of planar graphs that, for any integers $s$ and $t$, $G[L_s\cup\cdots\cup L_{s+t-1}]$ has treewidth at most $3t-1$ \cite{baker:approximation}.
 ...
 For each $i\in\{0,\ldots,\lfloor \log h\rfloor\}$ and each $j$, $G_{i,j}$ has treewidth at most $?9\cdot 2^i-1$ \david{should $?9$ be $3$}.  By \cref{weighted_separator}, with weight function $\zeta(v):=1$ and $c=\lceil |V(G^+_{i,j})|/(D2^{i-1})\rceil$, there exists a set $X_{i,j}$ of size  
 \[
    |X_{i,j}|\le 9\cdot 2^i\cdot(c-1) =
    9\cdot 2^i\cdot\left(\left\lceil\frac{ |V(G^+_{i,j})|}{D2^{i-1}}\right\rceil-1\right) \le 
    \frac{9\cdot 2^i\cdot|V(G^+_{i,j})|}{ D2^{i-1}} 
    = \frac{18|V(G^+_{i,j})|}{D} 
 \]
 ...
 \[
    ...
    \le
    \sum_{i=0}^{\lfloor\log h\rfloor-1}\sum_{j} \frac{18|V(G^+_{i,j})|}{D}
    \le
    \sum_{i=0}^{\lfloor\log h\rfloor-1}\frac{54n}{D}
    % \le
    % \frac{3(?3+1)n(1+\log h)}{D}
    % = O((n/D)\log n) 
    \le \frac{54\log h}{D}\le\frac{54\log n}{D}
    \enspace .
