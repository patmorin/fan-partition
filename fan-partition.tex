...
We are now ready to prove \cref{main_thm_products}. The entirety of this subsection should be treated as a proof of \cref{main_thm_products}. Most of the results in this section are written as Claims that are not self-contained, since they refer $G$, $H$, $P$, $X$, $d^*$, and other objects defined throughout this subsection. From this point on, $G$ is an $n$-vertex subgraph of $H\boxtimes P$ where $H$ is a \defin{$t$-tree} (an edge-maximal graph of treewidth $t$) and $P$ is a path.

We now outline the structure our proof.
...
\david{The variable `$a$' looks like the word `a'. Please use a different letter.} \pat{I'd prefer not to, and I don't think it's necessary. The only place in which the variable $a$ appeared naked inside a sentence was the following sentence, and I've added ${}>0$ to fix that.}
