...
Pathwidth is the standard measure of how similar a graph is to a path.}. \citet{distel.dujmovic.ea:product} show that the class of treewidth-$2$ graphs is sufficient.  In the current work, we show that one can even take $\mathcal{H}$ to be the class of pathwidth-$2$ graphs.  Even more specifically, every $n$-vertex planar graph is isomorphic to a $\tilde{O}(\sqrt{n})$-blowup of a fan.\footnote{A \defin{fan} $F_k$ is a graph with $V(F_k):=\{v_0,\ldots,v_k\}$ and $E(F_k):=\{v_0v_i:1\leq i\leq k\}\cup\{v_iv_{i+1}:1\leq i\leq k-1\}$. \david{Add the following? Note that $\{v_0,v_1,v_2\},\{v_0,v_2,v_3\},\dots,\{v_0,v_{k-1},v_k\}$ is a path-decomposition of $F_k$ with width 2.}}
\david{For brevity ``In the current work, we show that one can even take $\mathcal{H}$ to be the class of fan graphs, which have pathwidth $2$.''}
...
\item The fan in \cref{main_thm_planar} is of length $O(\sqrt{n}/\log^2 n)$. In general, if $\bw(G)\leq k$ then $G$ is contained in $P\boxtimes K_k$ where $|V(P)|=\ceil{|V(G)|/k}$.
\end{itemize}}
...
\end{compactenum}
Note that $k$-blowup of $H$ can be written as the strong product $H\boxtimes K_k$.  Therefore, \cref{main_thm_planar} states that, for every $n$-vertex planar graph $G$ there is a fan $F$ such that $G$ is isomorphic to a subgraph of $F\boxtimes K_{O(\sqrt{n}\log^2 n)}$. We prove the following more general result.

\david{Shall we define row treewidth here? The \defin{row treewidth} of a graph $G$, denoted by $\rtw(G)$, is the minimum integer $t$ such that $G$ is contained in $H\boxtimes P$ for some graph $H$ with treewidth $t$ and for some path $P$. Then \cref{main_thm_products} becomes: \\
...
\begin{thm}\label{main_thm_products}
  For every graph $H$ with treewidth $t$, for every path $P$, every $n$-vertex subgraph of $H\boxtimes P$ is contained in a $O(t\sqrt{n}\log^2 n)$-blowup of a fan.
\end{thm}
...
We follow a similar strategy by proving that, for a carefully chosen sparsifying set $X$, there is a metric space $(V(H\boxtimes P-X),d^*)$ that contracts the shortest paths metric on $H\boxtimes P-X$, preserves the local density of $G-X$, and has a $(k,O(\sqrt{\log n})$-volume-preserving Euclidean contraction.  (A generalization of) a result of Feige that relates bandwidth, local density, and volume-preserving Euclidean contractions then establishes \cref{main_thm_products}.  A key element in our proof is that $d^*$ mixes distances measured in $H\boxtimes P$ with distance increases caused by large obstacles in $X$. This contracts the shortest path metric on $G-X$ enough to prove the existence of a volume-preserving Euclidean contraction but not so much that it increases the local density.
...
A \defin{distance function} over a set $S$ is any function $d:S^2\to\R$ that satisfies $d(x,x)=0$ for all $x\in S$, $d(x,y)\ge 0$ and $d(x,y)=d(y,x)$ for all distinct $x,y\in S$, and $d(x,z) \le d(x,y)+d(y,z)$ for all distinct $x,y,z\in S$.  For any $x\in S$ and $S'\subseteq S$, $d(x,S'):=\min\{d(x,y):y\in S'\}$.  A \defin{metric space} $\mathcal{M}:=(S,d)$ consists of a set $S$ and a distance function $d$ over some superset of $S$. $\mathcal{M}$ is \defin{finite} if $S$ is finite.  For $x\in S$, the \defin{$r$-ball} centered at $x$ is $\mathdefin{B_{\mathcal{M}}(x,r)}:=\{y\in S:d(x,y)\le r\}$.

For any graph $G$, $d_G$ is a distance function over $G$ and $\mathcal{G}_G:=(V(G),d_G)$ is a metric space. Any metric space that can be defined this way is referred to as a \defin{graph metric}.
...
A finite metric space $\mathcal{M}:=(S,d)$ has \defin{local density} at most $D$ if $|B_\mathcal{M}(x,r)|\le 1+Dr$, for each $r\ge 0$ and each $x\in S$.
This definition is consistent with the definition of local density of graphs:  A connected graph $G$ has local density at most $D$ if and only if the metric space $\mathcal{M}_G$ has local density at most $D$.

A \defin{contraction} of a metric space $\mathcal{M}:=(S,d)$ into a metric space $\mathcal{M'}:=(S',d')$ is a function $\phi:S\to S'$ that satisfies $d'(\phi(x),\phi(y))\le d(x,y)$, for each $x,y\in S$.  For two points $x,y\in\R^L$ we let $\mathdefin{d_2(x,y)}$ denotes the Euclidean distance between $x$ and $y$.  A contraction of $(S,d)$ into $(\R^L, d_2)$ for some $L\ge 1$ is called a \defin{Euclidean contraction}.  For $K\subseteq S$ we abuse notation slightly with the shorthand $\mathdefin{\phi(K)}:=\{\phi(x):x\in K\}$.  When $S\subseteq S'$ and $\phi$ is the identity function we say that $\mathcal{M'}$ is a contraction of $\mathcal{M}$. In particular, the statement $(S,d')$ is a contraction of $(S,d)$ is equivalent to the statement $d'(x,y)\le d(x,y)$ for all $x,y\in S$. We use two easy observations that follow quickly from these definitions:
...
First, we need a definition of bandwidth for metric spaces.  Let $(S,d)$ be a metric space and let $x_1,\ldots,x_n$ be a permutation $S$.  Then $\mathdefin{\bw_{(S,d)}(x_1,\ldots,x_n)}:=\max\{j-i:d(x_i,x_j)\le 1\}$ and $\mathdefin{\bw(S,d)}$ is the minimum of $\bw_{(S,d)}(x_1,\ldots,x_n)$ taken over all $n!$ permutations $x_1,\ldots,x_n$ of $S$.  Note that this coincides with the definition of the bandwidth of a graph: For any connected graph $G$, $\bw(V(G),d_G)=\bw(G)$.  First we observe that injective contractions can only increase bandwidth:

\begin{obs}\label{contraction_increases_bandwidth}
  For any finite metric space $\mathcal{M}:=(S,d)$ and any contraction $\mathcal{M}':=(S,d')$ of $\mathcal{M}$, $\bw(\mathcal{M}) \le \bw(\mathcal{M}')$.
\end{obs}
...
\begin{lem}\label{planar_sparsifier}
  For each $D\le n$, every $n$-vertex planar graph $G$ has a set $X$ of $O((n/D)\log n)$ vertices such that $G-X$ has local density at most $D$.
\end{lem}
...
In this section, we prove \cref{main_thm_products}.  The structure of our proof is based on the proof of \cref{rao} by \citet{rao:small}, which applies to planar graphs and even to $K_{h}$-minor-free graphs for any fixed $h$.  However, the proof cannot be applied directly since graph products of the form $H\boxtimes P$ are not $K_h$-minor-free for any fixed $h$, even when $H$ is a tree and $P$ is a path. \pat{Say something about the ``tennis court'' example?} \david{If $T$ is the tree obtained from the path $P_{2n+1}$ by adding one leaf adjacent to the middle vertex, then $K_n$ is a minor of the `tennis court' graph $T\square P_{n^2}$ (see  \citep{Wood-NYJM11}). }
...
\begin{enumerate}
  ...
  \item Design a distance function $d^*:V((H\boxtimes P)-X)^2\to\R_{\ge 0}$ so that the metric space $\mathcal{M}^*:=(V(G-X),d^*)$ is a contraction of $\mathcal{M}$ that also has local density $O(D)$.

  Graphically, $\mathcal{M}_{G-X}\rightbroom\mathcal{M}\rightbroom\mathcal{M}^*$.

  \item Prove that $\mathcal{M}^*$ has an $(k,O(\sqrt{\log n}))$-volume-preserving Euclidean contraction $\phi:V(G-X)\to\R^L$, for $k\in\Theta(\log n)$.
...
The delicate part of the proof is the design of the distance function $d^*$ that contracts $d_{(H\boxtimes P)-X}$ but still ensures that the local density of $\mathcal{M}^*$ is $O(D)$. If $d^*$ contracts too much, then $\mathcal{M}^*$ will not have local density $O(D)$. If $d^*$ contracts too little, then it will be difficult to get a $(k,O(\sqrt{\log n}))$-volume-preserving Euclidean embedding. To make this work, we choose a highly-structured sparsifying set $X$.

\subsection{A Structured Sparsifier}

In this section we construct a sparsifying set $X$ like that used in \cref{planar_sparsifier}, but with a structure that lends itself to the design of the distance function $d^*$.
...
In words, $Q_{i,0},\ldots,Q_{i,N/2^i-1}$ partition the part of $H\boxtimes P$ that contains $G$ into vertex-disjoint strips of height $2^i$. The subgraph $G^+_{i,j}$ is a strip of height $3\cdot 2^i$ that contains $G_{i,j}$ in its middle third.  Observe that, for each $i\in\{1,\ldots,\log N\}$ and $j\in\{0,\ldots,N/2^i-1\}$, each of the graphs $Q^+_{i-1,2j}$ and $Q^+_{i-1,2j+1}$ is a subgraph of $Q^+_{i,j}$. Equivalently, $Q^+_{i,j}$ is a subgraph of $Q_{i+1,\lfloor j/2\rfloor}$ for each $i\in\{0,\ldots,\log N-1\}$ an each $j\in\{0,\ldots,N/2^i-1\}$.

We construct vertex subsets $Y_{i,j}$ of $H$ for each $i\in\{0,1,\ldots,\log N\}$ and $j\in\{0,\ldots,N/2^{i}-1\}$.  Define a weight function $w_{i,j}:V(H)\to\N$ where $w_{i,j}(x):=|(\{x\}\times V(P^+_{i,j})) \cap V(G)|$.  Observe that $W_{i,j}:=\sum_{x\in V(H)} w(x)=|V(Q^+_{i,j})\cap V(G)|$. By \cref{pq_separator}, there exists $Y_{i,j}\subseteq V(H)$ of size at most $c(t+1)W_{i,j}/(2^{i-1}D)$ such that, each component $C$ of $H-Y_{i,j}$ has total weight at most $\sum_{x\in V(C)} w(x) \le 2^{i-1}D$.  For each $i\in\{0,1,\ldots,\log N\}$ and $j\in\{0,\ldots,N/2^{i}-1\}$, let $\mathdefin{X_{i,j}}:=Y_{i,j}\times V(P^+_{i,j})$.
...
\pat{TODO: Reread and check for $\pm 1$ issues. Use $u$, $v$, and $w$ consistently.}
...
\begin{obs}\label{i_bounds}
  For each $o\in\{1,2\}$, if
  $i_o(v,w)\ge 0$ then $\delta_o(v,w)\le 2^{i_o(v,w)+1}$.
\end{obs}

Finally, define
\[
  \mathdefin{d^*(u,w)} := \max\{\delta_1(u,w),\, \delta_2(u,w),\, \delta_3(u,w),\, d_{H\boxtimes P}(u,w) \}
\]
...
\begin{proof}
  If $\delta_0(u,v)=2^i$ because, say $i_0(u,v)=i$, then any path in $(H\boxtimes P)-X$ from $u$ to $v$ must begin in the subgraph $Q_{i,j}$ that contains $u$ and must not be contained in the graph $Q^+_{i,j}$.  Therefore $d_{(H\boxtimes P)-X}(u,v) \ge 2^i=\delta_0(u,v)$.
...
\begin{proof}
  ...
  Next we consider the case where $d^*(u,w)=\delta_0(u,w)$.
  In this case $i_0(u,w)\ge 0$ and assume, without loss of generality that $i:=i_0(u,w)=i_0^{\shortrightarrow}(u,w)$.  Let $j$ be such that $Q_{i,j}$ contains $u$.  Then $u$ and $w$ are in different components, $C_u$ and $C_w$ of $Q^+_{i,j}-X_{i,j}$.  We distinguish between two cases depending on the location of $v$.  If $v$ is not a vertex of $Q^+_{i,j}$, then $d_{(H\boxtimes P)-X}(u,v) \ge d_{H\boxtimes P}(u,v) \ge 2^i=\delta_0(u,w)$ and there is nothing to prove.  Otherwise, $v$ is contained in some component $C_v$ of $Q^+_{i,j}-X_{i,j}$.  If $C_v\neq C_u$ then $i_0(u,v)\ge i$ so $d^*(u,v)\ge 2^i$ and again there is nothing to prove.  If $C_v=C_u$, then $d^*(u,v)+d^(v,w)\ge d_{H\boxtimes P}(u,v)+\delta_o(u,v)\ge 2^i=d^*(u,w)$ for some $o\in\{0,1,2\}$.

  Finally, we consider the case where $d^*(u,w)=\delta_1(u,w)$. Then $i:=i_1(u,w)\ge 0$, so $u$ and $w$ are both contained in $G_{i,j}$ but in different components $C_u$ and $C_w$ of $G_{i,j}-X_{i,j+1}$, for some $j$.  If $v$ is not a vertex of $G_{i,j}$ then $d_{H\boxtimes P}(u,v)+d_{H\boxtimes P}(v,w) \ge \delta_1(u,w)$ and we are done.  If $v$ is a vertex of $C_u$, then $d_{H\boxtimes P}(u,v)+\delta_1(u,v)\ge \delta_1(u,w)$. The case where $v$ is a vertex of $C_w$ is symmetric.  If $v$ is neither a vertex of $C_u$ nor a vertex of $C_w$, then $\delta_1(u,v) + \delta_1(v,w) \ge \delta_1(u,w)$.
\end{proof}
...
\begin{lem}
  For every integer $k\in\{1,\ldots,n\}$, the metric space $\mathcal{M}^*:=(V(G-X),d^*)$ has a $(k,O(\sqrt{\log n}))$-volume-preserving Euclidean embedding. \pat{TODO: Check quantifier on $k$}
\end{lem}
...
\begin{proof}
  Let $C$ be a component of $H\boxtimes P-Z$.  For any two vertices $x:=(x_1,x_2)$ and $y:=(y_1,y_2)$ of $C$, $d_{C}(x,y) = \max\{d_{H-Z_H}(x_1,y_1),d_{P}(x_2,y_2)$ where $Z_H$ is the result of running $\textsc{Chop}_{\Delta,t}(H)$. By \cref{component_diameter_h},  $d_{H-Z_H}(x_1,y_1)\le ?t\Delta$.  By the choice of $Y_0$, $d_{P}(x,y)<\Delta$.
\end{proof}
...
Let $Z:=Z_{\Delta,t}(H,P)$ be a subset of $V(H\boxtimes P)$ generated by running the procedure described above, for some $\Delta\in\{1,\ldots,|V(P)|\}$.\footnote{The assumption $\Delta\le |V(P)|$ ensures that $Z$ is non-empty, so that $d_{H\boxtimes P}(x,Z)$ is well-defined, for all $x\in V(H\boxtimes P)$.}  Consider the graph $I:=\mathdefin{I(Z)}$ obtained from $Q\boxtimes P$ by removing all edges incident to at least one vertex in $Z$.

For each component $C$ of $I$, let $X_C:=\cup\{X_{i,j}:C\subseteq Q^+_{i,j}\}$ and let $J$ be the subgraph of $I$ obtained by removing, for each component $C$ of $I$, the vertices in $X_C\cap V(C)$.

\begin{lem}
  If $v$ and $w$ are in the same component of $J$, then $d^*(v,w)\le ??\Delta$.
\end{lem}

\begin{proof}
  Let $v$ and $w$ be two vertices in the same component $C'$ of $J$ that is contained in some component $C$ of $I$.   Then $v$ and $w$ are in the same component of $I$ so, by \cref{component_diameter}, $d_{H\boxtimes P}(v,w)\le ?t\Delta \le ??\Delta$.

  If $\delta_0(v,w)=2^i>\Delta$ then, for some $j$, $v\in V(Q_{i,j})$ and $w\in V(Q^+_{i,})$ and $v$ and $w$ are in different components of $Q_{i,j}-X_{i,j}$.  Since $2^i>\Delta$, $Q^+_{i,j}$ contains $C$ so $X_C$ contains $X_{i,j}$. But this would imply that $v$ and $w$ are in different components of $C-X_C$, contradicting the assumption that $v$ and $w$ are both in $C'$.

  If $\delta_1(v,w)>?\Delta$ then $v$ and $w$ are vertices of $Q_{i,j}$ that are in different components of $X_{i,j+1}$. Since $\delta_1(v,w)>?\Delta$ and $C$ contains $v$ and $w$, $Q^+_{i,j+1}$ contains $C$. Again, this would imply that $v$ and $w$ are in different components of $C-X_C$, contradicting the assumption that $v$ and $w$ are both in $C'$.

  The situation in which $\delta_1(v,w)>?\Delta$ is handled symmetrically.
\end{proof}
...
For each component $C'$ of $J$, choose a uniformly random $\alpha_{C'}$ in $[0,1]$. For each $v \in V(C')$, let $\varphi_{Z}(v):=(1+\alpha_{C'})\,d^+_{H\boxtimes P}(v,Z)$.  (Note that this definition uses positive distance $d^+_{H\boxtimes P}$.)
...
\begin{lem}
  For any $v,w\in V(J)$,
  \[
    |\varphi_Z(v)-\varphi_Z(w)| \le 2 d^*(v,w) \enspace .
  \]
\end{lem}

\begin{proof}
  % If $v$ and $w$ are in different components of $I$ then
  % \begin{align*}
  %    |\varphi_Z(v)-\varphi_Z(w)|
  %   & =|(1+\alpha_i)d_{H\boxtimes P}(v,Z)-(1+\alpha_j)d_{H\boxtimes P}(w,Z)| \\
  %   & \le 2\max\{d_{H\boxtimes P}(v,Z), d_{H\boxtimes P}(Z,w)\} \\
  %   & \le 2\left(d_{H\boxtimes P}(v,Z) + d_{H\boxtimes P}(Z,w)\right) \\
  %   & \le 2d_{H\boxtimes P}(v,w) \le 2d_{(H\boxtimes P)-X}(v,w) \enspace ,
  % \end{align*}
  % where the penultimate inequality follows from the fact that every path in $H\boxtimes P$ from $v$ to $w$ contains a vertex of $Z$.
  % We now assume that $v$ and $w$ are in the same component of $I$.
  %
  % If $v$ and $w$ are in the same component $C_i$ of $I-X$ then
  % \begin{align*}
  %   |\varphi_Z(v)-\varphi_Z(w)|
  %   & =(1+\alpha_i)|d_{H\boxtimes P}(v,Z)-d_{H\boxtimes P}(w,Z)| \\
  %   & \le 2|d_{H\boxtimes P}(v,Z)-d_{H\boxtimes P}(w,Z)| \\
  %   & \le 2 d_{H\boxtimes P}(v,w) \le 2d_{(H\boxtimes P)-X}(v,w) \enspace ,
  % \end{align*}
  % where the penultimate inequality comes rewriting the triangle inequalities $d(v,Z)\le d(v,w)+d(w,Z)$ and $d(w,Z)\le d(w,v)+d(v,Z)$.
  %
  % If $v$ and $w$ are in different components of $(H\boxtimes P)-X$ then $d_{(H\boxtimes P)-X}(v,w)=\infty$ and the result is trivial.
  %
  % The only remaining possibility is that $v$ and $w$ are in the same component $C$ of $I$ but in different components $C_i$ and $C_j$ of $I-X$.
  % % Then $C=H'\boxtimes P'$ where $H'$ is an induced subgraph of $H$ whose diameter is at most $?t\Delta$ and $P'$ is a subpath of $P$ with $\Delta-1$ vertices.
  % Then every path from $v$ to $w$ in $(H\boxtimes P)-X$ contains at least one vertex of $Z$.  Therefore, $d_{(H\boxtimes P)-x}(v,w)\ge d_{H\boxtimes P}(v,Z)+d_{H\boxtimes P}(Z,w)$.
  %
  %
  %
  %
  %
  % If $v$ and $w$ are in different components $C_i$ and $C_j$ of $I-X$ then
  % \begin{align*}
  %   \begin{align*}
  %     |\vaphi_Z(v)-\varphi_Z(w)|
  %     & =|(1+\alpha_i)d_{H\boxtimes P}(v,Z)-(1+\alpha_j)d_{H\boxtimes P}(w,Z)| \\
  %     & \le 2\max\{d_{H\boxtimes P}(v,Z), d_{H\boxtimes P}(Z,w)\} \\
  %     & \le 2\left(d_{H\boxtimes P}(v,Z) + d_{H\boxtimes P}(Z,w)\right)
  %     & \text \\
...
%   For any two vertices $x$ and $y$ of $H\boxtimes P$, $|\varphi_Z(x)-\varphi_Z(y)|\le 2d_{(H\boxtimes P)-X}(x,y)$.
% \end{lem}
%
% \begin{proof}
%   \pat{TODO: Fix this proof.} For brevity, let $d(\cdot,\cdot):=d_{(H\boxtimes P)-X}(\cdot,\cdot)$ and $d^+(\cdot,\cdot):=d^+_{(H\boxtimes P)-X}(\cdot,\cdot)$ throughout this proof.
...
The following lemma says that $\phi$ is almost a Euclidean contraction.  In a final step, we will divide each coordinate of $\phi$ by $2\sqrt{L}$ to obtain a Euclidean contraction but, until then, it is more convenient to work directly with $\phi$.

\begin{lem}\label{euclidean_contraction}
  For each $v,w\in V((H\boxtimes P)-X)$, $d_2(\phi(v),\phi(w)) \le 2\sqrt{L}\cdot d_{(H\boxtimes P)-X}(v,w)$.
\end{lem}

\begin{proof}
  By \cref{double_distance}, $|\phi_{i,j}(v)-\phi_{i,j}(w)|\le 2d^*(v,w)$.  Therefore \pat{Finish by computing Euclidean distance.}
\end{proof}
...
The rest of the analysis in this section closely follows \citet{rao:small}, which in turn closely follows \citet{feige:approximating} with the subtle modifications noted above.  We proceed slowly and carefully since our setting is significantly different (we are working in a subgraph $G$ of $H\boxtimes P$ but using the metric space $(V(G),d^*)$) and we expect that some readers will not be familiar with some methods introduced by \citet{feige:approximating} that are only sketched by \citet{rao:small}.
...
\begin{proof}
  ...
  By \cref{uniform} $\phi_{i,j}(v_p)$ is uniformly distributed over an interval of length at least $\Delta/10(t+1)$, for each $j\in J$.  We claim that the location of $\phi_{i,j}(v_p)$ in this interval is independent of the corresponding coordinate, $x_{i,j}$, of $x$.  There are two cases to consider:
  \begin{compactenum}
    \item If $d_{H\boxtimes P}(v_p,\{v_1,\ldots,v_{p-1}\})= h$, either $\Delta=1$ or \cref{component_diameter} implies that the component of $(H\boxtimes P)-Z$ that contains $v_p$ does not contain any of $v_1,\ldots,v_{p-1}$. In either case, the component of $I(H,P,Z_{i,j})$ that contains $v_p$ does not contain any of $v_1,\ldots,v_{p-1}$.  Therefore, the location of $\phi_{i,j}(v_p)$ is determined by a random real number $\alpha\in[0,1]$ that does not contribute to $\phi(v_1),\ldots,\phi(v_{p-1})$.  Since $(\lambda_1,\ldots,\lambda_{p-1})=\lambda(\phi(v_1),\ldots,\phi(v_{p-1}))$ is completely determined by $\phi(v_1),\ldots,\phi(v_{p-1})$, $\alpha$ is independent of $x=\sum_{k=1}^{p-1}\lambda_k\phi(v_k)$.  In particular, $\alpha$ is independent of $x_{i,j}$.

    \item Otherwise, $d^*(v_p,\{v_1,\ldots,v_{p-1}\})=h >d_{H\boxtimes P}(v_p,\{v_1,\ldots,v_{p-1}\})$.  For each $q\in\{1,\ldots,p-1\}$, $d^*(v_p,v_q)\ge h>d_{H\boxtimes P}(v_p,v_q)$.  Therefore $d^*(v_p,v_q)$ is one of $\delta_0(v_p,v_q)$, $\delta_1(v_p,v_q)$, or $\delta_2(v_p,v_q)$.  By definition, this means that for some (maximum) $i'$ and some $b$, $v_p$ and $v_q$ are both vertices of $Q^+_{i',b}$, but $v_p$ and $v_q$ are in different components of $Q_{i',b}-X_{i',b'}$ for some $b'\in\{b-1,b,b+1\}$.  By \cref{i_bounds} and the definition of $\delta_0$, $h=d^*(v_p,v_q)\le 2^{i'+1}$, so $2^{i'}\ge h/2 \ge h/?\ge 2^i$. Therefore $i'\ge i$.  Therefore, $v_p$ and $v_q$ are in different components of $Q_{i,j}-X_{i',b'}$ and therefore in different components of $Q_{i,j}-X$.  Therefore, the location of $\phi_{i,j}(v_p)$ is determined by a random real number $\alpha\in[0,1]$ that does not contribute to $\phi(v_1),\ldots,\phi(v_{p-1})$.  As above, this implies that $\alpha$ is independent of $x_{i,j}$.
  \end{compactenum}
