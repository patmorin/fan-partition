...
Let $Z(u,v)$ be the set of all $i,j$ such that $u$ and $v$ are vertices of $Q^+_{i,j}$ that are in different components of $Q^+_{i,j}-X_{i,j}$.  Define
\[
  d^*(u,v):=\max\left(\{d_{H\boxtimes P}(u,v)\}\cup \left\{\min_{x\in V(P)\setminus V(P^+_{i,j})} (d_P(u,x) + d_P(x,v))\right\}\right)
\]
Intuitively, $d^*(u,v)$ captures the fact that any path from $u$ to $v$ in $(H\boxtimes P)-X$ must navigate around each obstacle $X_{i,j}$ that separates $u$ and $v$ in the graph $Q^+_{i,j}$.  At the very least, this requires a path from $u$ to some vertex $x$ outside of $Q^+_{i,j}$ followed by a path from $x$ to $v$.


height of the largest obstacle
...
\begin{proof}
  ...
  % \pat{The previous sentence is where the "otherwise" clause in the definition of $i^\rightarrow_0$ matters.  Specifically, }
  % \gwen{I am not sure where this "otherwise" is in that definition, do you mean "but $u$ and $w$ are in different components of $Q^+_{i,\lfloor a/2^i\rfloor}-X_{i,\lfloor a/2^i\rfloor}$"} \pat{No, the ``otherwise'' clause I'm referring to occurs when $u$ and $w$ are in the same component of $G^+_{i,j}-X_{i,j}$ for every value of $i$.  This happens, for example, when the metric space $(V(G),d_{H\boxtimes P})$ already has local density at most $D$ so each set $X_{i,j}$ is empty. Then $\delta_0(u,w)=0$ for every $u,w\in V(G)$ so the lemma, as stated, is not true.  In this case, we have to use the fact that $d^*(u,w)\ge d_{H\boxtimes P}(u,w)$.}
...
By \cref{delta_density}, $\delta_0$ defines a space $(V(G),\delta_0)$ with local density $O(D)$.  Unfortunately, this is not a metric space because $\delta_0$ does not necessarily satisfy the triangle inequality.  The remaining functions used to define $d^*$ ensure that $d^*$ is a metric space.
...
Finally, define
\[
  \mathdefin{d^*(u,v)} := \max\{\delta_0(u,v),\; \delta_1(u,v),\; \delta_2(u,v),\; d_{H\boxtimes P}(u,v) \}
\]
First we argue that $d^*$ is a contraction of $d_{(H\boxtimes P)-X}$.
\begin{lem}\label{d_star_contraction}
  For every pair of vertices $u,v\in V((H\boxtimes P)-X)$, $d^*(u,v)\le d_{(H\boxtimes P)-X}(u,v)$.
\end{lem}
...
\begin{proof}
  ...
  If $d^*(u,w)=\delta_0(u,w)$ then assume, without loss of generality that $i:=i_0(u,w)=i_0^{\shortrightarrow}(u,w)$.  Let $j:=\floor{a/2^i}$, so that $Q_{i,j}$ contains $u$ and $Q^+_{i,j}$ contains $w$.  Since $i=i^{\shortrightarrow}_0(u,w)$, $u$ and $w$ are in different components, $C_u$ and $C_w$ of $Q^+_{i,j}-X_{i,j}$ \pat{or $Q^+_{i,j}$ contains $u$ but does not contain $w$.}.  We distinguish between two cases depending on the location of $v$.  If $v$ is not a vertex of $Q^+_{i,j}$, then $d^*(u,v) \ge d_{H\boxtimes P}(u,v) \ge 2^i=\delta_0(u,w)=d^*(u,w)$ and there is nothing to prove.
  \gwen{I got confused by the first inequality: Did you mean to write $d^*(u,v) \ge d_{H\boxtimes P}(u,v)$?}\pat{Yes, fixed now.}
  Otherwise, $v$ is contained in some component $C_v$ of $Q^+_{i,j}-X_{i,j}$.  If $C_v\neq C_u$ then $i_0(u,v)\ge i$ so $d^*(u,v)\ge 2^i$ and again there is nothing to prove.  If $C_v=C_u$, then $d^*(u,v)+d^*(v,w)\ge d_{H\boxtimes P}(u,v)+\delta_b(v,w)\ge 2^i=d^*(u,w)$ for some $b\in\{0,1,2\}$. (The choice of $b$ depends on the location of $v$; if $v\in V(Q_{i,j})$ then $b=0$, if $v\in V(Q_{i,j-1})$ then $b=1$, and if $v\in V(Q_{i,j+1})$, then $b=2$.)

  Since $\delta_1$ and $\delta_2$ are symmetric, all that remains is to consider the case where $d^*(u,w)=\delta_1(u,w)$. In this case, $i:=i_1(u,w)\ge 0$, so $u$ and $w$ are both contained in $Q_{i,j}$ but are in different components $C_u$ and $C_w$ of $Q_{i,j}-X_{i,j+1}$, for some $j$.
  \begin{compactitem}
      ...
      \item If $v$ is a vertex of $Q_{i,j+1}$ then, since $u$ and $w$ are in different components of $Q_{i,j}-X_{i,j+1}$, they are also in different components of $Q^+_{i,j+1}-X_{i,j+1}$.  At least one of these components, say the component that contains $w$, does not contain $v$.  Thus $v$ and $w$ are in different components of $Q^+_{i,j}-X_{i,j+1}$, which implies that $i^{\shortrightarrow}_0(v,w)\ge i$, so $d^*(v,w)\ge \delta_0(v,w)\ge 2^i$.
...
\begin{proof}
  ...
  \todo[inline]{It might be helpful to introduce the notation $d_P(v,w)$ and $d_H(v,w)$ for vertices in $H\boxtimes P$. }
  ...
  % and $\delta_1(v,w)=d_P(v_2,x_2)+d_P(x_2,w_2)$ for some vertex $x:=(x_1,x_2)\in V(H\boxtimes P)\setminus V(Q^+_{i,j+1})$.  For any vertex $y:=(y_1,y_2)$ in $C$, $d_P(v,y),d_P(y,w) <\Delta$.  Therefore, $x$ is not a vertex of $C$, since
  % $3\Delta>\delta_1(v,w)=d_P(v,x_2)+d_P(x_2,w)$.  By \cref{i_bounds}, $3\Delta<\delta_1(v,w)\le 2^{i+1}$, which implies that $Q^+_{i,j+1}$ contains $C$.\todo{This explanation could be better.}  Therefore $X_C$ contains $X_{i,j+1}$.  Again, this contradicts the assumption that $v$ and $w$ are in the same component of $J$.
