...
We say that $\mathcal{M}$ is \defin{finite} if $S$ is finite.  A finite metric space $\mathcal{M}:=(S,d)$ has \defin{local density} at most $D$ if $|B_\mathcal{M}(x,r)|\le Dr$ for each $r\ge 1$ and each $x\in S$.  It will be convenient for us to work with a discrete version of local density:  A finite metric space $\mathcal{M}:=(S,d)$ \defin{local density} at most $D$ if $|B_\mathcal{M}(x,r)|\le Dr$ for each \emph{integer} $r\ge 1$ and each $x\in S$.
Both definitions are consistent with the definition of local density of graphs:  A connected graph $G$ has local density at most $D$ if and only if the metric space $(V(G),d_G)$ has (discrete) local density at most $D$.
...
\citet[Theorem~10]{feige:approximating} proves the following result for graph metrics, which establishes a critical connection between local density and tree volume:

\begin{lem}[{\citet[Theorem~10]{feige:approximating}}]\label{reciprocal_sum}
  For any $n$-element metric space $(S,d)$ with local density at most $D$ and minimum distance $\min_{\{x,y\}\in\binom{S}{2}} d(x,y)\ge 1$, and any positive integer $k$,
  \[
...
\begin{proof}
  ...
  \begin{equation}
    \sum_{y\in S\setminus\{x\}} \frac{1}{d(x,y)} \le 2D\ln(n/D) \enspace . \label{reciprocal_crux}
  \end{equation}
  ...
  \begin{equation}
    \sum_{K\in\binom{S}{k}}\sum_{\pi\in\Pi(K)}\frac{1}{\prod_{i=1}^{k-1}d(\pi(i),\pi(i+1))} \le n(2D(1+\ln(n/D)))^{k-1} \enspace .
    \label{volume_sum}
  \end{equation}
  ...
  \begin{align*}
    ...
    & \le \sum_{K'\in\binom{S}{k-1}}\sum_{\pi\in\Pi(K')}\frac{1}{\prod_{i=1}^{k-2}d(\pi(i),\pi(i+1))}\cdot 2D(1+\ln(n/D)) & \text{(by \cref{reciprocal_crux})}\\
    & \le n(2D(1+\ln(n/D)))^{k-2}(2D(1+\ln(n/D)) & \text{(by induction)}\\
    & = n(2D(1+\ln(n/D)))^{k-1} \enspace . & & \qedhere
  \end{align*}
\end{proof}



The following Theorem is proved implicitly by \citet{feige:approximating} for graph metrics. We recap the proof here for metric spaces and do so in order to make the relationship between the parameters $n$, $D$, and $\eta$.

\begin{thm}
  Let $(S,d)$ be a $n$-element metric space with local density at most $D$ and minimum-distance $\min_{\{x,y\}\in\binom{S}{2}}d(x,y)=1$, and let $\phi:S\to \R^L$ be a $(k,\eta)$-volume respecting Euclidean contraction of $(V(G),d_G)$. Then $\bw(S,d) \in O(Dk\eta\log^{3/2} n)$.
\end{thm}

\todo[inline]{Define a version of bandwidth for metric spaces. For a layout $x_1,\ldots,x_n$, $\bw(x_1,\ldots,x_n)=\max\{j-i:d(x_i,x_j)=1\}$. Actually, I think this is just the minimum distortion of a Euclidean embedding of $\phi:S\to\Z$.}
\todo[inline]{There must be a lower bound of $\Omega(\log n)$ on $k$ in this theorem/proof. Figure out where it's used.}

\begin{proof}
  Let $r$ be a random unit vector in $\R^L$ and for each $v\in S$, let $h(v):=\langle r,\phi(v)\rangle$ be the inner product of $r$ and $\phi(v)$.  We will order the elements of $S$ as $v_1,\ldots,v_n$ so that $h(v_1)\le \cdots \le h(v_n)$.  To prove an upper bound on $\bw(S,d)$, it suffices to upper bound the maximum, over all $vw$ with $d(v,w)=1$, of the number of vertices $x$ such that $h(v)\le h(x)\le h(w)$.

  Consider some pair $v,w\in S$ with $d(v,w)=1$. Since $\phi$ is a contraction, $d_2(\phi(v),\phi(w))\le 1$.  By \cite[Proposition~7]{feige:approximating}, $\Pr(|h(v)-h(w)|\ge \sqrt{4a\ln n/L}) \le n^{-a}$, for any $a>0$. Therefore, with probability at least $1-n^{-a+2}$, $|h(v)-h(w)|\le \sqrt{4a\ln n/L}$ for each edge $vw$ of $G$.

  Let $K$ be a $k$-element subset of $S$ and let $\ell(K):=\max_{v\in K}h(v)-\min_{v\in K} h(v)$.  By \cite[Theorem~9]{feige:approximating} there exists a universal constant $\beta$ such that, for any $c\ge 1$,
  \[
      \Pr(\ell(K) \le c)
  ...
  \end{align}
  Let $B$ a maximum cardinality subset of $V(G)$ with $\ell(B)<\sqrt{4a\ln n/L}$.  These $B$ vertices define $\binom{B}{k}$ bad $k$-element sets. Therefore, by Markov's Inequality, the probability that $\binom{B}{k}$ exceeds \eqref{bad_expectation} by a factor of at least $2$ is at most $1/2$.  Therefore, with probability at least $1/2$, $\binom{|B|}{k}\le 2\cdot\eqref{bad_expectation}$, which implies that $|B|\in O(Dk\eta\log^{3/2} n)$ with probability at least $1/2$.\footnote{Very roughly, $\binom{|B|}{k}$ is approximated by $(|B|/k)^k$.}  Therefore, with probability at least $1/2-n^{-a+2}$, $\bw(x_1,\ldots,x_n)\in O(Dk\eta\log^{3/2} n)$, so $\bw(S,d)\in O(Dk\eta\log^{3/2} n)$.
\end{proof}
