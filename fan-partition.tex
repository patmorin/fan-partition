...
\begin{lem}
  For each $(a,b)\in\Z^2$, each component $C$ of $(H\boxtimes P)^\Delta_{a,b}$ has $d_{H\boxtimes P}(C)\le 2\Delta+1$.
\end{lem}
...
\begin{proof}
  ...
  To do this, we make use of the following property of $t$-trees \cite{kundgen.pelsmajer:?}:\pat{Provide a proof or reference.} For any integer $s$, the set $N_C$ of vertices in $L_{s}$ that are adjacent to a single component $C$ of $H[L_{s+1}]$ form a clique in $H$.
...
\begin{proof}
  Let $v:=(v_1,v_2)$.
  With probability at least $1/2$, $v_2$ is in the middle half of $P_b$.  With probability at least $1/2$, $v_1$ is in the middle half of the layers that define $H^\Delta_a$.
\end{proof}
