...
A \defin{contraction} of a metric space $\mathcal{M}:=(S,d)$ into a metric space $\mathcal{M'}:=(S',d')$ is a function $\phi:S\to S'$ that satisfies $d'(\phi(x),\phi(y))\le d(x,y)$, for each $x,y\in S$.  For two points $x,y\in\R^\ell$ we let $\mathdefin{d_2(x,y)}$ denotes the Euclidean distance between $x$ and $y$.  A contraction of $(S,d)$ into $(\R^\ell, d_2)$ for some $\ell\ge 1$ is called a \defin{Euclidean contraction}.  For $K\subseteq S$ we abuse notation slightly with the shorthand $\mathdefin{\phi(K)}:=\{\phi(x):x\in K\}$.  When $S\subseteq S'$ and $\phi$ is the identity function we say that $\mathcal{M'}$ is a contraction of $\mathcal{M}$. In particular, the statement $(S,d')$ is a contraction of $(S,d)$ is equivalent to the statement $d'(x,y)\le d(x,y)$ for all $x,y\in S$.
...
\begin{enumerate}
  ...
  \item Prove that $\mathcal{M}^*$ has an $(k,O(\sqrt{\log n}))$-volume-preserving Euclidean embedding $\phi:V(G-X)\to\R^L$, for $k\in\Theta(\log n)$.
...
\begin{proof}
  ...
  Now let $\mathdefin{X}:=\bigcup_{i=0}^{\lfloor\log_2 n\rfloor}\bigcup_{j=0}^{N/2^i-1} X_{i,j}$.  Then $|X|\le\sum_{i=0}^{\log N}\sum_{j=0}^{N/2^i-1} |X_{i,j}|\le 18c(t+1)n(1+\log N)/D\le 18c(t+1)n(2+\log n)/D$.
...
Let $u:=(x_1,y_a)$ and $w:=(x_3,y_c)$ be two vertices of $H\boxtimes P$. Define the function $i^{\shortrightarrow}:V(H\boxtimes P)\setminus X\to\Z$ as follows:
If $a=c$ and $v$ and $w$ are in the same component of $H\boxtimes\{y_a\}-X$ then $i^{\shortrightarrow}(u,w):=-\infty$.  Otherwise, let $i^{\shortrightarrow}(u,w)$ be the maximum integer $i$ such that no component of $Q^+_{i,\lfloor a/2^i\rfloor}-\bigcup_{p=i}^{\log N} X_{i,\lfloor a/2^p\rfloor}$ contains both $u$ and $w$. Let $i^*(v,w):=\max\{i^{\shortrightarrow}(u,w),i^{\shortrightarrow}(w,u)\}$. Define:
\[
...
Ultimately, $d^*$ will be the maximum of several functions, one of which is $\delta_1$.  Of these functions, $\delta_1$ is the most important.  It ensures that the local density of the metric space $\mathcal{M}^*$ has local density $O(D)$.

\begin{lem}\label{delta_density}
  For each $v\in V(H\boxtimes P)$ and $r\ge 0$,
  \[
    |B_{(V(G),\delta_1)}(v,r):=\{w\in V(G):\delta_1(v,w)\le r\}| \le 3Dr \enspace .
  \]
\end{lem}
...
By \cref{delta_density}, $\delta_1$ defines a space $(V(G),\delta_1)$ with local density $O(D)$.  Unfortunately, this is not a metric space because $\delta_1$ does not satisfy the triangle inequality.  The remaining functions used to define $d^*$ are ensure that $d^*$ is a metric space.

Let $v:=(x_2,y_b)$.  We now define a second quantity $\delta_2(v,w)$.  There are two cases:
\begin{compactenum}[(i)]
    \item If there exists some $i\in\{0,\ldots\log N\}$ such that $v,w\in V(G_{i,j})$ but $v$ and $w$ are in different components of $G_{i,j}-X_{i,j+1}$ then consider the maximum such $i$ and set $\delta_2(v,w)=a-j2^i + b-j2^i$.
    \item If not such $i$ exists, then $\delta_2(v,w):=0$.
\end{compactenum}

A third quantity $\delta_3(v,w)$ is defined symmetrically:
\begin{compactenum}[(i)]
    \item If there exists some $i\in\{0,\ldots\log N\}$ such that $v,w\in V(G_{i,j})$ but $v$ and $w$ are in different components of $G_{i,j}-X_{i,j-1}$ then consider the maximum such $i$ and set $\delta_3(v,w)=(j+1)2^i-a + (j+1)2^i-b$.
    \item If not such $i$ exists, then $\delta_3(v,w):=0$.
\end{compactenum}

Finally, define
\[
  d^*(u,w) := \max\{\delta_1(u,w),\, \delta_2(u,w),\, \delta_3(u,w),\, d_{H\boxtimes P}(u,w) \}
\]
...
\begin{proof}
  Show that each of $\delta_1$, $\delta_2$, and $\delta_3$ is dominated by $d_{(H\boxtimes P)-X}$.
\end{proof}
...
\begin{proof}
  ...
  In this case $i^*(u,w)\ge 0$ and assume, without loss of generality that $i:=i^*(u,w)=i^{\shortrightarrow}(u,w)$.  Let $j:=\floor{a/2^i}$.  Since $w\not\in X$, one of the two cases must occur:
  \begin{compactenum}
...
\begin{lem}
  For every integer $k\in\{1,\ldots,n\}$, the metric space $\mathcal{M}^*:=(V(G-X),d^*)$ has a $(k,O(\sqrt{\log n}))$-volume-preserving Euclidean embedding. \pat{Check quantifier on $k$}
\end{lem}
...
In the following lemma, and for the rest of this section, we use the convention that for any vertex $x$ of $H$, $d_H(x,\emptyset):=\infty$.  \pat{Come back to this later!}
...
Let $a$ be a constant whose value will be lower-bounded later.  We now define a random function $\phi:V(G)\to\R^{L}$ where $L:=\lfloor 1+\log_2 n\rfloor\cdot\lceil a k\ln n\rceil$. For each $i\in\{0,\ldots,\log N-1\}$ and each $k\in\{0,\ldots,N/2^i$ we consider each component $C$ of $G_{i,k}-X$.  For each $j\in\{1,\ldots,\lceil a k\ln n\rceil\}$, let $Z_{i,k}(C)\sim Z_{\Delta,t}(H,P)$ be the random subset of $V(H\boxtimes P)$ obtained by using the procedure described above with parameters $\Delta=2^i$ and $t$ and with all random choices made independently.  For each $x\in V(G_{i,k})$, let $\phi_{i,j}(x):= \varphi_{Z_{i,j}}(x)$ again with all choices made independently.  Finally,
\[
