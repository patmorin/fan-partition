...
\david{why are the affiliations commented out?}
  ...
  \begin{tabular}{|c|c|c|c|l|} \hline
   ...
   & tree & $O(\Delta\cdot\tw(G))$ & $\Omega(\mathcolor{red}{\Delta^{1/3}}\cdot\tw(G))$ & \cite{LMST08,ding.oporowski:some} \\
    ...
    $K_t$-minor-free & tw~$4$ & $O_t(\sqrt{n})$ & $\Omega(t\sqrt{n})$ & \cite{distel.dujmovic.ea:product} \\
        $K_{3,t}$-minor-free & tw~$2$ & $O(t\sqrt{n})$ &  & \cite{distel.dujmovic.ea:product} \\
    genus-$g$ & fan & $O(\sqrt{gn}+\sqrt{n}\log^2 n)$ & $\Omega(\sqrt{gn})$ & \cite{DEW17}, \cref{main_thm_genus} \\
    genus-$g$ & tw~$2$ & $O((g+1)\sqrt{n})$ &  & \cite{distel.dujmovic.ea:product} \\
...
\david{Where does the lower bound $\Omega(\sqrt{\tw(Q)\cdot n})$ come from?}
...
\begin{lem}\label{k_planar_density}
  Every $n$-vertex $k$-planar graph has $O(k^{1/2} n)$ edges.\david{This lemma is due to \citet{PachToth97}}
\end{lem}
...
\begin{restatable}[{Generalization of result in \cite{feige:approximating}}]{thm}{volumedensitybandwidth}\label{volume_density_bandwidth}
...
\begin{figure}
    ...
    \end{tabular}
    \caption{The definitions of $\delta_1$, $\delta_2$, and $\delta_3$. \pat{Fix this figure!}}
    \label{d_star}
\end{figure}
