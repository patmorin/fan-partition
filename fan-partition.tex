...
\pat{TODO: Write a corollary of \cref{main_thm_products} that lists some of the graph classes with product structure.}
...
The \defin{local density} of a graph $G$ is $\mathdefin{\ld(G)}:=\max\{(|B(v,r)|-1)/r:r\ge 0,\, v\in V(G)\}$.\footnote{The ${}-1$ in this definition of local density does not appear in the definitions of local density used in some other works \cite{feige:approximating,rao:small}, but this makes no difference to our asymptotics results and will be convenient for us, since it allows us to consider balls of radius $r\ge 0$ rather than just balls of radius $r\ge 1$.} The local density of $G$ provides a lower bound on the bandwidth of $G$.  If $|B_G(v,r)|=Dr+1$ then in any permutation $v_1,\ldots,v_n$ of $V(G)$, one of the vertices $w$ in $B_G(v,r)\setminus \{v\}$ is at distance at least $Dr/2$ from $v$. That is, $v=v_i$ and $w=v_j$, and $|j-i|\ge Dr/2$.  Since $G$ contains a path with at most $r$ edges from $v$ to $w$, one of the edges $v_{i'}v_{j'}$ of this path must have $|i'-j'|\ge Dr/2r=D/2$.  Therefore $\bw(G)\ge\ld(G)/2$.  In his seminal work, \citet{feige:approximating} proves that bandwidth is also upper bounded by the local density and a polylogarithmic function of the number of vertices.

\begin{thm}[\citet{feige:approximating}]\label{feige_bandwidth_vs_density}
  For every $n$-vertex graph $G$, $\bw(G)\in O\left(\ld(G)\cdot \log^3 n\sqrt{\log n\log\log n}\right)$.
\end{thm}

Rao improves \cref{feige_bandwidth_vs_density} in the special case of planar graphs:\footnote{\cref{rao_bandwidth_vs_density} is not stated explicitly in \cite{rao:small}, but follows from the existence of $(k,O(\sqrt{\log n})$-volume-preserving Euclidean contractions and a result of \citet{feige:approximating} that bounds bandwidth for graphs with local density $D$ that have $(k,\eta)$-volume-preserving Euclidean contractions as a function of $n$, $D$, $k$, and $\eta$.  A generalization of Feige's result is stated in \cref{volume_density_bandwidth}.}

\begin{thm}[\citet{rao:small}]\label{rao_bandwidth_vs_density}
  For every $n$-vertex planar graph $G$, $\bw(G)\in O\left(\ld(G)\cdot \log^3 n\right)$.
\end{thm}

By \cref{rao_bandwidth_vs_density}, to prove \cref{main_thm_planar} it is sufficient to show that every $n$-vertex planar graph $G$ has a vertex subset $X$ of size $O(\sqrt{n}\log^2 n)$ such that $\ld(G-X)=O(\sqrt{n}/\log n)$.  A generalization of this is given in \cref{planar_sparsifier}.  However, to prove \cref{main_thm_products}\ldots \pat{Continue here.}
...
For a graph $G$ and any two vertices $v,w\in V(G)$, the \defin{distance in $G$} between $v$ and $w$, denoted $\mathdefin{d_G(v,w)}$, is the minimum number of edges in any path from $v$ to $w$ in $G$, or $d_G(v,w):=\infty$ if $v$ and $w$ are in different connected components of $G$.  If $G$ is connected, then $d_G$ is a distance function over $V(G)$ so $\mathdefin{\mathcal{M}_G}:=(V(G),d_G)$ is a metric space. Any metric space that can be defined this way is referred to as a \defin{graph metric}.  For $v\in V(G)$ and $W\subseteq V(G)$, $\mathdefin{d_G(v,W)}:=\min\{w\in W:d_G(v,w)\}$. The \defin{positive distance in $G$} between $v$ and $W$ is $\mathdefin{d^+_G(v,W)}:=\max\{1,d_G(v,W)\}$.
